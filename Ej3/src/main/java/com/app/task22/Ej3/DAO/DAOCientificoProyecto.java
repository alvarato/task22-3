...
public class DAOCientificoProyecto extends DAO{
	...
	public ArrayList<CientificoProyecto> showAll() throws SQLException{
		String query = "SELECT p.id_proyecto, p.nombre_proyecto, p.horas,c.dni,c.nom_apels FROM cientificos c LEFT JOIN asignado_a  ON asignado_a.cientifico = c.dni LEFT JOIN proyecto p ON asignado_a.proyecto = p.id_proyecto ORDER BY p.id_proyecto ;";
		...
		while (resultado.next()) {
			...
			Cientifico cf = new Cientifico(dni, nomApel);
